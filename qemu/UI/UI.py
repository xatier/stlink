...
class Ctrl:
    def __init__(self,parent):
        ...
        self.thread_green=threading.Thread(target=self.fct_thread,
                                           args=("green",self.ui.entryPortVerte,self.serversocketVerte,self.connVerte,self.addrVerte))
        self.thread_green.setDaemon(True)
        self.thread_blue=threading.Thread(target=self.fct_thread,
                                           args=("blue",self.ui.entryPortBleue,self.serversocketBleue,self.connBleue,self.addrBleue))
        self.thread_blue.setDaemon(True)
        ...
        while 1:
            ...
            if conn == self.connVerte :
                if ord(data) == 0:
                    ...
                    self.ui.log.insert(Tkinter.END,"Data " + str(ord(data)) + " received on" + " connection green\n")
                elif ord(data) == 1:
                    self.ui.canvas.itemconfigure(self.ui.ledDroite, image=self.ui.ledVerte)
                    self.ui.log.insert(Tkinter.END,"Data " + str(ord(data)) + " received on" + " connection green\n")
                else:
                    self.ui.log.insert(Tkinter.END,"Data " + str(ord(data)) + " received on" + " connection green => unknown\n")
            elif conn == self.connBleue :
                if ord(data) == 0:
                    self.ui.canvas.itemconfigure(self.ui.ledGauche, image=self.ui.ledBlanche)
                    self.ui.log.insert(Tkinter.END,"Data " + str(ord(data)) + " received on" + " connection blue\n")
                elif ord(data) == 1:
                    self.ui.canvas.itemconfigure(self.ui.ledGauche, image=self.ui.ledBleue)
                    self.ui.log.insert(Tkinter.END,"Data " + str(ord(data)) + " received on" + " connection blue\n")
                else:
                    self.ui.log.insert(Tkinter.END,"Data " + str(ord(data)) + " received on" + " connection blue => unknown\n")
    ...
    def launch_server(self,name,entryPort,serversocket,conn,addr):
        ...
        if name == "green" :
            try:
                self.ui.log.insert(Tkinter.END,"Waiting for connection on port " + entryPort.get()+ " for LED Green\n")
                self.ui.update()
                ...
                print serversocket
        if name == "blue" :
            self.ui.log.insert(Tkinter.END,"Waiting for connection on port " + entryPort.get()+ " for LED Blue\n")
            self.ui.update()
            conn, addr = serversocket.accept()
            self.serversocketBleue = serversocket
            self.connBleue = conn
        if name == "bouton" :
            self.ui.log.insert(Tkinter.END,"Waiting for connection on port " + entryPort.get()+ " for bouton\n")
            self.ui.update()
            conn, addr = serversocket.accept()
            self.serversocketBouton = serversocket
            self.connBouton = conn
        self.ui.log.insert(Tkinter.END,"Connected \n")
        self.ui.update()
